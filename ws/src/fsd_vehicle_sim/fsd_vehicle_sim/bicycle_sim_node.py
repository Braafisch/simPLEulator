...
class BicycleSim(Node):
    ...
    def __init__(self):
        ...
        # -------- IO --------
        self.sub_v = self.create_subscription(Float32, '/target_speed', self.on_speed, 10)
        self.sub_d = self.create_subscription(Float32, '/steering_angle', self.on_steer, 10)
        ...
        self.get_logger().info('bicycle_sim ready: subs [/target_speed, /steering_angle] → pub [/odom, TF map→base_link]')
    ...
    def step(self):
        ...
        self.publish_ground_speed(now.to_msg(), self.v, self.yaw)
    ...
    def publish_gps(self, stamp_msg):
        ...
        navsat.position_covariance = [pos_var_xy, 0.0, 0.0,
                                   0.0, pos_var_xy, 0.0,
                                   0.0, 0.0, pos_var_alt]
