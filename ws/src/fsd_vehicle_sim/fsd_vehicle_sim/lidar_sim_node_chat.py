...
class OS1LikeLidar(Node):
    def __init__(self):
        ...
        self.declare_parameter("accept_only_frame", "map")  # e.g. 'map' or ''
        ...
        self.declare_parameter(
            "dual_return", False
        )  # if True, emit second hit when available

        # ---- Ranging limits / masks
        self.declare_parameter("min_range", 0.2)
        self.declare_parameter(
            "max_range", 90.0
        )  # ~typical @10% target; set 200 for max reach
        ...
        )  # simple 1st-order decay with range
    ...
    def _cb_cones(self, msg: MarkerArray):
        ...
        if self.accept_only and src_frame and src_frame != self.accept_only:
            self.get_logger().warn_once(
                f'Ignoring cones in frame "{src_frame}" (accept_only="{self.accept_only}")'
            )
            return
    ...
    def _transform_point(self, p_src, src_frame):
        ...
        try:
            tp = self.tf_buffer.transform(
                ps, self.lidar_frame, timeout=rclpy.duration.Duration(seconds=0.05)
            )
    ...
    def _transform_cones_to_lidar(self):
        ...
        try:
            _ = self.tf_buffer.lookup_transform(
                self.lidar_frame, self._cones_src_frame, Time()
            )
        except Exception:
            return []
        out = []
    ...
    def _on_timer(self):
        ...
        for ring_idx, elev in enumerate(self.vert_angles):
            ...
            for cx, cy, cz in cones_lidar:
                ...
                t_hit = self._raycast_cylinder(
                    cx, cy, cz, self.cone_r, self.cone_h, dx, dy, dz
                )
                if t_hit is None:
                    continue
            ...
            rng_noisy = max(
                self.min_range, t_best + random.gauss(0.0, self.range_noise)
            )
            x = rng_noisy * dx
            y = rng_noisy * dy
            z = rng_noisy * dz
            ...
            time_off = self._col_idx * self.col_dt + (ring_idx / self.R) * (
                self.col_dt * 0.2
            )

            # Append point (x,y,z,intensity,ring,time)
            self._scan_points.append(
                (x, y, z, float(intensity), ring_idx, float(time_off))
            )
            ...
            if self.dual_return:
                ...
                for cx2, cy2, cz2 in cones_lidar:
                    if cone_best and (cx2, cy2, cz2) == cone_best:
                        ...
                        t2 = self._raycast_cylinder_second(
                            cx2, cy2, cz2, self.cone_r, self.cone_h, dx, dy, dz, t_best
                        )
                        if t2 is not None:
                            rng2 = t2
                            zt2 = rng2 * dz
                            if (
                                self.min_range <= rng2 <= self.max_range
                                and self.min_z <= zt2 <= self.max_z
                            ):
                                if t_second is None or rng2 < t_second:
                                    t_second = rng2
                        continue
                    # other cones further out
                    t2 = self._raycast_cylinder(
                        cx2, cy2, self.cone_r, self.cone_h, dx, dy, dz
                    )
                ...
                if t_second is not None:
                    rng2n = max(
                        self.min_range, t_second + random.gauss(0.0, self.range_noise)
                    )
                    x2 = rng2n * dx
                    y2 = rng2n * dy
                    z2 = rng2n * dz
                    intensity2 = (
                        max(0.0, self.I0 - self.I_decay * rng2n) * 0.7
                    )  # weaker 2nd return
                    time_off2 = time_off + 1e-4
                    self._scan_points.append(
                        (x2, y2, z2, float(intensity2), ring_idx, float(time_off2))
                    )
    ...
    def _advance_column_and_maybe_publish(self):
        if self._col_idx == 0 and not self._scan_points:
            self.get_logger().warn_throttle(
                2000, "Keine LiDAR-Treffer in letzter Umdrehung (prüfe TF/Z-Fenster)!"
            )
        self._col_idx += 1
