...
class AccelTrackMarkers(Node):
    def __init__(self):
        ...
        self.declare_parameter('lane_half_width', 1.5)  # halber Spurabstand (links/rechts) [m]
        ...
        self.lane_half_width  = self.get_parameter('lane_half_width').get_parameter_value().double_value
    ...
    def build_markers(self) -> MarkerArray:
        ...
        lifetime = Duration(sec=0, nanosec=0)  # 0 = bleibt bestehen, bis gelöscht/überschrieben
        ...
        def make_cone(mid: int, x: float, y: float, color, ns: str) -> Marker:
            ...
            m.ns = ns
            m.id = mid
            m.type = Marker.CYLINDER        # simple Geometrie; alternativ: MESH_RESOURCE mit Cone-Mesh
            m.action = Marker.ADD
        ...
        for i in range(n):
            x = i * self.spacing
            ma.markers.append(make_cone(2*i,   x, -self.lane_half_width, yellow, 'cones_left'))
            ma.markers.append(make_cone(2*i+1, x,  self.lane_half_width, blue,   'cones_right'))
